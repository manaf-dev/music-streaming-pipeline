terraform {
  ...
  # The backend bucket must be created out-of-band before the first
  # `terraform init`. Use a name unique to your account, e.g.
  # music-pipeline-tfstate-<account_id>.
  backend "s3" {
...
locals {
  ...
  # Predicted ARNs — feed IAM policies before the corresponding resources exist.
  ...
  # Absolute paths to source-tree assets — resolved from this env directory.
...
# ---------------------------------------------------------------------------
# Module composition (dependency order: S3 -> IAM+DynamoDB -> Glue -> SFN)
# ---------------------------------------------------------------------------
module "s3" {
  source = "../../modules/s3"
